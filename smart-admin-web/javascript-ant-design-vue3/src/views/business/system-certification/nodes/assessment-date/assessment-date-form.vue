...
-->
<template>
    <a-modal title="评定日期" width="400px" :open="visibleFlag" @cancel="onClose" :maskClosable="false"
        :destroyOnClose="true">
        <a-form ref="formRef" :model="form" :rules="rules" :label-col="{ span: 8 }">
            <a-row>
                <a-col :span="24">
                    <a-form-item label="评定日期" name="assessmentDate">
                        <a-date-picker valueFormat="YYYY-MM-DD" v-model:value="form.assessmentDate" style="width: 100%"
                            placeholder="评定日期" />
                    </a-form-item>
                </a-col>
            </a-row>
...
const formDefault = {
    ...
    nodeId: NODE_CONST.assessment_date, //节点ID
    nodeStatus: undefined, //节点状态
    passReason: undefined, //跳过原因
    assessmentDate: undefined, //资料上报日期
};
