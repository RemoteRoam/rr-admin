...
<template>
    ...
    <a-form class="smart-query-form">
        <a-row class="smart-query-form-row">
            <a-form-item label="销售单ID" class="smart-query-form-item">
                <a-input style="width: 150px" v-model:value="queryForm.salesId" placeholder="销售单ID" />
            </a-form-item>
    ...
    <a-card size="small" :bordered="false" :hoverable="true">
        ...
        <SalesItemForm ref="formRef" @reloadList="queryData" />
...
import SalesItemForm from './sales-item-form.vue';
// ---------------------------- 表格列 ----------------------------
...
async function queryData() {
    ...
    try {
        let queryResult = await salesItemApi.queryPage(queryForm);
...
async function requestDelete(data) {
    ...
    try {
        ...
        await salesItemApi.delete(data.id);
...
async function requestBatchDelete() {
    try {
        SmartLoading.show();
        await salesItemApi.batchDelete(selectedRowKeyList.value);
        message.success('删除成功');
        queryData();
    } catch (e) {
        smartSentry.captureError(e);
