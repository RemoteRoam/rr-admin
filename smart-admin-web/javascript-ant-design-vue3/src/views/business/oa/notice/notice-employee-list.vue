...
<template>
  ...
  <a-card size="small" :bordered="false">
    ...
    <a-table rowKey="noticeId" :columns="tableColumns" :dataSource="tableData" :scroll="{ x: 1500 }" :pagination="false"
      :loading="tableLoading" bordered size="small">
      <template #bodyCell="{ column, record, text }">
        <template v-if="column.dataIndex === 'title'">
          ...
          <span v-show="!record.viewFlag">
            <a @click="toDetail(record.noticeId)">【{{ record.noticeTypeName }}】{{ text }}
              <span style="color: red">（未读）</span>
            </a>
          </span>
        </template>
        <template v-if="column.dataIndex === 'pageViewCount'"> {{ record.userViewCount }}人 / {{ record.pageViewCount }}次
        </template>
      </template>
    </a-table>
