...
<template>
  <a-form class="smart-query-form" v-privilege="'oa:notice:query'">
    <a-row class="smart-query-form-row">
      ...
      <a-form-item label="发布时间" class="smart-query-form-item">
        <a-range-picker v-model:value="publishDate" :presets="defaultTimeRanges" @change="publishDateChange"
          style="width: 220px" />
      </a-form-item>

      <a-form-item label="创建时间" class="smart-query-form-item">
        <a-range-picker v-model:value="createDate" :presets="defaultTimeRanges" @change="createDateChange"
          style="width: 220px" />
      </a-form-item>
  ...
  <a-card size="small" :bordered="false">
    ...
    <a-table rowKey="noticeId" :columns="tableColumns" :dataSource="tableData" :scroll="{ x: 1510 }" :pagination="false"
      ...
      <template #bodyCell="{ column, record, text }">
        ...
        <template v-else-if="column.dataIndex === 'action'">
          <div class="smart-table-operate">
            ...
            <a-button type="link" @click="onDelete(record.noticeId)" v-privilege="'oa:notice:delete'"
              danger>删除</a-button>
          </div>
        </template>
      </template>
    ...
    <div class="smart-query-table-page">
      <a-pagination showSizeChanger showQuickJumper show-less-items :pageSizeOptions="PAGE_SIZE_OPTIONS"
        ...
        :total="total" @change="queryNoticeList" @showSizeChange="queryNoticeList"
