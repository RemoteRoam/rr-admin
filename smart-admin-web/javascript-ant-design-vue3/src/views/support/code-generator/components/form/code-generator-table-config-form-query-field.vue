...
<template>
  ...
  <a-table size="small" bordered id="smartCodeQueryFieldsTable" class="smart-margin-top10" :dataSource="tableData"
    row-class-name="column-row" :columns="columns" rowKey="rowKey" :pagination="false">
    <template #bodyCell="{ record, index, column }">
      ...
      <template v-if="column.dataIndex === 'queryTypeEnum'">
        <SmartEnumSelect @change="(value) => onChangeQueryType(value, record)"
          enumName="CODE_QUERY_FIELD_QUERY_TYPE_ENUM" v-model:value="record.queryTypeEnum" width="100%" />
      </template>
      ...
      <template v-if="column.dataIndex === 'columnNameList'">
        <a-select show-search
          ...
          v-model:value="record.columnNameList" @change="onSelectColumn(record)" style="width: 100%">
